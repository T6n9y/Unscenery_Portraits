...
function Home() {
  ...
  // Array of images for the collage (replace with your images)
  const images = [
      ...
      <div className="container mx-auto px-4 py-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-8">
          {images.map((image) => (
            <div
              key={image.id}
              className="cursor-pointer overflow-hidden rounded-lg shadow-lg bg-[#ffdfba] transition-all duration-500 ease-in-out transform hover:scale-105"
              onClick={() => setSelectedImage(image.src)} // Set the selected image on click
            >
              <img
                src={image.src}
                alt={image.alt}
                className="w-full h-64 object-cover rounded-lg transition-all duration-500 ease-in-out transform"
              />
            </div>
          ))}
        </div>
